...
def _post_process_outputs(tokenizer, output, *, require_retained_support=False):
    ...
    batched_output_topk_gumbels = []
    batched_output_token_ids = []
    batched_logprobs = []
    for output_token_ids, log_probs, idx, gumbel in out_map:
        batched_output_topk_ids.append(idx)
        batched_output_topk_gumbels.append(gumbel)
...
class SGLangRollout(BaseRollout):
    def __init__(
        ...
        assert not (
                not config.enforce_eager and config.free_cache_engine), "disable CUDA graph (enforce_eager = False) if free cache engine"
    ...
    def _init_inference_engine(self, trust_remote_code, actor_module, port):
        ...
        if arithmetic in ("native_fa3_v1", "native_fa3_v2"):
            if self._tp_size != 1:
                raise ValueError("native Qwen replay requires TP1")
            engine_options.update(opd_qwen_replay_backend=arithmetic, attention_backend="fa3", device="cuda", disable_radix_cache=True)
        if arithmetic == "native_fa3_v2":
            engine_options["random_seed"] = int(self.config.get("seed", 11)) + self._rank
        ...
        if first_rank_in_node:
            ...
            self._engine = AsyncEngine(
                ...
                enable_soft_thinking=self.config.get("enable_soft_thinking", True),
                add_noise_dirichlet=self.config.get("add_noise_dirichlet", False),
                add_noise_gumbel_softmax=self.config.get("add_noise_gumbel_softmax", False),
                sampling_backend=self.config.get("sampling_backend", 'flashinfer'),
                max_topk=self.config.get("top_k", 5),
    ...
    def _init_sampling_params(self, **kwargs):
        kwargs = dict(
            ...
            after_thinking_temperature=self.config.get("after_thinking_temperature", 1),
            after_thinking_top_p=self.config.get("after_thinking_top_p", 0.95),
            after_thinking_top_k=self.config.get("after_thinking_top_k", 10),
            after_thinking_min_p=self.config.get("after_thinking_min_p", 0.0),
            think_end_str=self.config.get("think_end_str", '</think>'),
            gumbel_softmax_temperature=self.config.get("gumbel_softmax_temperature", 1),
            early_stopping_entropy_threshold=self.config.get("early_stopping_entropy_threshold", 0.0),
            early_stopping_length_threshold=self.config.get("early_stopping_length_threshold", 256),
            noise_factor=self.config.get("noise_factor", 1),
            noise_gaussian=self.config.get("noise_gaussian", False),
            noise_gumbel=self.config.get("noise_gumbel", True),
            noise_on_logits=self.config.get("noise_on_logits", True),
            noise_on_inputs=self.config.get("noise_on_inputs", False),
    ...
    @torch.no_grad()
    def generate_sequences(self, prompts: DataProto, **kwargs) -> DataProto:  ## Place for generation
        ...
        batch = TensorDict(
            {
                ...
                "rollout_topk_ids": rollout_topk_ids,  # we will recompute old log prob with actor
                "rollout_topk_gumbels": rollout_topk_gumbels,  # we will recompute old log prob with actor
                "attention_mask": attention_mask,
                "position_ids": position_ids,
                "gumbel_temperature": torch.full(
                    (batch_size,),
                    float(self.config.get("gumbel_softmax_temperature", 0.5)),
    ...
    async def _handle_engine_call(self, _req: AsyncRolloutRequest, do_sample: bool, is_validate: bool,
                                  override_n: bool = True, **kwargs) -> dict:
    ...
    def _req_level_generate_sequences(self, prompts: DataProto, **kwargs) -> DataProto:
        ...
        for req in sorted_output_req_list:
            ...
            assert len(req.input_ids) == len(req.attention_mask) == len(req.position_ids) == len(
                req.loss_mask), f"""Request {req.request_id} has different length of 
        ...
        if prompt_attention_mask.shape[1] < self.config.prompt_length:
            prompt_attention_mask = pad_sequence_to_length(prompt_attention_mask, self.config.prompt_length, 0,
                                                           left_pad=True)
        ...
        if prompt_position_ids.shape[1] < self.config.prompt_length:
            prompt_position_ids = pad_sequence_to_length(prompt_position_ids, self.config.prompt_length, 0,
                                                         left_pad=True)
