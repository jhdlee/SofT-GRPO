...
class TokenizerManager:
    ...
    async def _handle_batch_request(
        ...
        if getattr(obj, "parallel_sample_num", 1) == 1:
            if self.server_args.enable_tokenizer_batch_encode:
                ...
                # Sequential tokenization and processing     _     go there
                for i in range(batch_size):
                    tmp_obj = obj[i]
    ...
    def _handle_batch_output(
        ...
        for i, rid in enumerate(recv_obj.rids):
            ...
            # for name, arr in [
            #     ("logprobs_val", recv_obj.output_token_logprobs_val[i]),
            #     ("topk_probs", recv_obj.output_topk_probs_list[i]),
            #     ("topk_gumbel", recv_obj.output_topk_gumbel_list[i]),
            #     ("topk_indices", recv_obj.output_topk_indices_list[i]),
            # ]:
            #     print(name, 'shape:', len(arr), len(arr[0]) if len(arr) > 0 else 0, end=' ')
            # print(' ')
            ...
            if self.enable_soft_thinking:
                ...
                meta_info["output_topk_gumbel_list"] = (
                    recv_obj.output_topk_gumbel_list[i]
                )
