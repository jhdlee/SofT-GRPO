...
def authenticate_checkpoint(root, step, *, arm_id, teacher=True):
    from verl.trainer.ppo.ray_trainer import _verify_checkpoint
    manifest = _verify_checkpoint(str(Path(root) / f"global_step_{step}"))
...
def compare_checkpoints(left, right, *, include_teacher):
...
class ProductionController:
    def __init__(self, args):
        ...
        self.report = {"schema_version": 1, "arm_id": args.arm, "status": "running",
                       ...
                       "wandb_run_id": self.row["wandb_run_id"], "phases": {}}
    ...
    def invoke(self, phase, *, resume_from_path=None):
        ...
        environment.update(WANDB_RUN_ID=run_id, WANDB_MODE="online", WANDB_RESUME="allow" if resume_from_path else "never",
                           ...
                           WANDB_PROJECT=PRODUCTION_PROJECT if phase == "production" else PRODUCTION_PROJECT + "-prologue",
                           OPD_PRODUCTION_STARTED=str(time.time()))
        if phase == "production":
            command.append("trainer.requeue_signal_file=" + json.dumps(str(self.args.signal_file)))
        invocation = {"phase": phase, "command": command, "wandb_run_id": run_id,
        ...
        self.report["phases"][phase] = {"status": "running", "log": str(log_path), "output": str(output), "wandb_run_id": run_id}
        self.persist()
    ...
    def admission(self):
        ...
        authenticate_checkpoint(split_root, 1, arm_id=self.args.arm)
        self.invoke("resume", resume_from_path=split_root / "global_step_1")
        left = authenticate_checkpoint(self.row["phases"]["uninterrupted"]["run_dir"], 2, arm_id=self.args.arm)
        right = authenticate_checkpoint(self.row["phases"]["resume"]["run_dir"], 2, arm_id=self.args.arm)
        parity = compare_checkpoints(left, right, include_teacher=True)
        ...
        if self.args.arm.startswith("softgrpo_math_opd"):
            ...
            authenticate_checkpoint(self.row["phases"]["full_dose"]["run_dir"], 1, arm_id=self.args.arm)
            self.invoke("zero_dose")
            baseline = authenticate_checkpoint(self.row["phases"]["zero_dose"]["run_dir"], 1, arm_id="softgrpo_math_s11")
            hybrid = authenticate_checkpoint(self.row["phases"]["uninterrupted"]["run_dir"], 1, arm_id=self.args.arm)
            result["zero_dose_parity"] = compare_checkpoints(baseline, hybrid, include_teacher=False)
    ...
    def run(self):
        ...
        try:
            ...
            checkpoint = authenticate_checkpoint(self.row["phases"]["production"]["run_dir"], step, arm_id=self.args.arm)
            if step == 109:
            ...
            write_json(self.root / "continuation.json", {
